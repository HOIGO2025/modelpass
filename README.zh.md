...
| 副本 | 方式 | 采集主机能毁掉它吗 |
|---|---|---|
| `scripts/pull_backup.sh`(笔记本/NAS) | **拉取** | **不能** |
| Cloudflare R2(`R2_REMOTE`) | 推送 | 能 —— 主机持有写密钥 |
| rsync 到另一台服务器(`BACKUP_HOST`) | 推送 | 能 —— 同上 |

最后一列就是全部要点。**推送型备份的安全上限就是那台推送的机器**:入侵、勒索软件、误 `rm -rf`、账号被封 —— 这些同时带走推送目标。从一台"主机连不上"的机器拉取,是唯一能全部幸存的副本。

它也是笔记本合盖就停的那一份,所以不能取代 R2。两个都要:**拉取的那份不可被毁,R2 那份无人值守也在跑。**
